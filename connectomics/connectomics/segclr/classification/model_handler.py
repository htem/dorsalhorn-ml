...
# Define your custom save function
def custom_save_function(model, filepath, model_config):
    # Custom logic to save the model, and access additional arguments like model_config
    print(f"Saving model to {filepath}...")
    print(f"Model configuration: {model_config}")
    save_model(model, model_config, filepath)

# Define the custom callback
class CustomModelCheckpoint(tf.keras.callbacks.Callback):
    def __init__(self, filepath, monitor='val_loss', mode='min', custom_save_fn=None, model_config=None, threshold=0.5):
        super().__init__()
        ...
        self.model_config = model_config  # Store the custom model_config argument
        self.threshold = threshold

    def on_epoch_end(self, epoch, logs=None):
        # Get the value of the monitored metric (validation sparse categorical accuracy or any other)
        ...
        # Only save if the accuracy exceeds the threshold
        if current < self.threshold:
            print(f"Epoch {epoch+1}: {self.monitor} = {current:.4f} (below threshold of {self.threshold}), not saving model.")
            return

        # Compare with the best value if accuracy is above the threshold
        if (self.mode == 'max' and current > self.best) or (self.mode == 'min' and current < self.best):
            ...
            # Call the custom save function if the metric improved
            if self.custom_save_fn:
                self.custom_save_fn(self.model, self.filepath, self.model_config)  # Pass model_config
            else:
                # Default save if no custom save function is provided
                print("Saving model with improved validation accuracy (default save).")
...
def train_model(
    ...
    random_seed: int = 42,
    save_model_path: str = 'best_model',
  ...
  compile_config = dict(
      loss=loss, metrics=["SparseCategoricalAccuracy"], optimizer=optimizer, weighted_metrics=[])
...
#   final_layer = model.layers[-1]  # Get the last layer
#   print("Final layer output shape:", final_layer.output_shape)
#   fit_config["class_weight"] = {0: 1.0, 1: 1.0, 2: 1.0, 3: 2.0}
  ...
  custom_callback = CustomModelCheckpoint(
      filepath=save_model_path,  # Path where the best model will be saved
      monitor='val_sparse_categorical_accuracy',     # Metric to monitor
      mode='max',             # Save model when val_loss improves
      custom_save_fn=custom_save_function,  # Your custom save function
      model_config=model_config  # Pass your custom model configuration
  )

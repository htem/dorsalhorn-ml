...
if __name__ == "__main__":    
    ...
    if config["CREATE_NEW_DATASET"]: 
        ...
        train_data_files, test_data_files = convert_neurons_to_embedding_files(train_neurons, test_neurons, 
                                                                   ...
                                                                   use_all_synapses_for_test=False)
    else:
    ...
    # # Create confusion matrices and save model TF
    # class_names_used = list(label_map.keys())
    # if 'cltmrunlabeled' in class_names_used:
    #     class_names_used.remove('cltmrunlabeled')
    # if 'adhtmrp' in class_names_used:
    #     class_names_used.remove('adhtmrp')
    # class_names_used = ['htmrp' if c=='chtmrp' else c for c in class_names_used]
    # class_names_used = ['den_syn' if c=='dend_synapses' else c for c in class_names_used]
    ...
    # # create plots for visual
    # create_graphs(train_labels, test_labels, predicted_classes_train, predicted_classes_test, class_names_used, train_history, added_str=add_str)
